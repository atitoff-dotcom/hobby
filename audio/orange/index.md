# Руководство по сборке HiEnd сетевого аудио-транспорта
...
Для вашей системы (где Orange Pi PC+ работает в режиме I2S Slave, а всю работу по бесшовному пересчету частот выполняет аппаратный ASRC внутри ADAU1467) системный файл /etc/asound.conf должен быть максимально «прозрачным».
Его главная задача — полностью отключить встроенные программные микшеры Linux (dmix), которые любят принудительно урезать звук до 48 кГц, и разрешить плеерам передавать аудиопоток напрямую в аудиоконтроллер Bit-Perfect (бит-в-бит) вплоть до 24/192.
## Правильный и чистый /etc/asound.conf
Откройте файл в консоли Armbian (sudo nano /etc/asound.conf) и пропишите следующую конфигурацию:

```
# Назначаем устройство по умолчанию
pcm.!default {
    type plug
    slave.pcm "direct_i2s"
}
...
# Конфигурация прямого аппаратного вывода без программного ресемплинга
pcm.direct_i2s {
    type hw
    card 1
    device 0
    # Разрешаем передачу данных в оригинальном аудио-формате
    format S24_LE
}
```

(Примечание: Убедитесь через команду aplay -l, что ваша I2S-карта действительно имеет индекс card 1. Если она отображается как card 0, то в строках card 1 замените единицу на ноль).
## В чем ключевое отличие от статьи на MYSKU?

* В статье автора заставлял принудительно работать плагин type rate с жесткой строкой rate 48000. Это заставляло процессор Orange Pi тратить силы на пересчет всех файлов в 48 кГц.
* В вашем конфиге используется type hw (Hardware). Это означает, что если Squeezelite или Kodi открывает фильм 24 бит / 192 кГц, Linux без единого изменения транслирует эти 192 кГц прямо на пины GPIO. Аудиоконтроллер Orange Pi PC+ будет послушно принимать частоту BCLK/LRCK от Master-порта ADAU1467, а блок ASRC внутри DSP аккуратно выровняет поток под ваш генератор.

Вы хотите сразу прописать параметры запуска Squeezelite под этот конфиг или сначала проверите работоспособность карты базовой утилитой aplay?
